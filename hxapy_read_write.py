...
def read_metas(f, silent=True):
    ...
    if mtype >= hxa.HXA_MDT_COUNT:
        print(
            f"HXA Error: File {f.name} has meta data of type {mtype}. There is only {hxa.HXA_MDT_COUNT} \
              types of meta data\n"
        )
        exit()
...
def layer_stack(count, f, silent: bool = True):
    ...
    for i in range(layer_count):
        ...
        layer_dictionary = layer_dict(
            name_length, name, components, hxa.hxa_data_type(dtype), data
        )
...
def convert_hxa_to_dict(filename, silent=True):
    try:
        f = open(filename, "rb")
    except OSError:
