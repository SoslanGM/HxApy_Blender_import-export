    ...
    if (meta_type == hxa.HXA_MDT_NODE):
        for al in range(meta['array_length']):
            if (meta['data'][al] >= count):
                if not silent:
                    print(f"HxA Verify Error: Node {node} has meta data {meta['name']} that is referencing a non \
...
def hxa_util_validate(hxa_file, silent=True):
    for nc in range(hxa_file['node_count']):
        ...
        if (node_type == hxa.HXA_NT_GEOMETRY):
            ...
            if (components != 3):
                if not silent:
                    print(f"HxA Verify Error: Node {mc} vertex layer vertex layer has {components} components. \
            ...
            if (name != hxa.HXA_CONVENTION_HARD_BASE_VERTEX_LAYER_NAME):
                if not silent:
                    print(f"HxA Verify Error: Node {nc} vertex layer is named {name}. \
            ...
            if (node['content']['corner_stack']['layer_count'] != 0):
                ...
                if (name != hxa.HXA_CONVENTION_HARD_BASE_CORNER_LAYER_NAME):
                    if not silent:
                        print(f"HxA Verify Error: Node {nc} reference layer is named {name}. Must be \
                ...
                for cc in range(node['content']['edge_corner_count']):
                    ...
                    if (reference >= node['content']['vertex_count']):
                        if not silent:
                            print(f"HxA Verify Error: Node {nc} has a reference value referencing a non existing \
                                  vertex ({reference}).\n")
                        return False
                ...
                if (face_count != poly_count):
                    if not silent:
                        print(f"HxA Verify Error: Node {nc} claims to have {face_count} faces but the reference data \
